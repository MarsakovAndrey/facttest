...
?>
<div class="element-id" id="<?=$arResult['ID']?>">
    <?
    /*     //ЭТО ОБНУЛЕНИЕ ТАБЛИЦЫ РЕЙТИНГА НЕ ТРОГАТЬ!!!! ЗАПУСТИТЬ ТОЛЬКО ОДИН РАЗ
    $arSelect = Array("ID", "NAME");
    $arFilter = Array("IBLOCK_ID"=>2, "ACTIVE_DATE"=>"Y");
    ...
    {
        $idprd=$ob['ID'];
        $queryResult = $connection->query("INSERT INTO marsakov_prod_raiting (id, activeviev, passiveviev,countbuy,countphoto,raiting) VALUES ('$idprd', 0, 0,0,0,0)");
        echo "<pre>";
       var_dump($ob);
       echo "</pre>";
    }
*/?>
    ...
    if (in_array($arResult['ID'],$_SESSION['history'])){?>
        <input class="saveviev" type="hidden" id="no-start">
        <?
    }
    else{
        $_SESSION['history'][]=$arResult['ID'];//сессия с просмотренными товарами
        ?>
        <input class="saveviev" type="hidden" id="yes-start">
        <?
    } ?>
    ...
    $arItemIDs = Array();//После этого ищем все корзины с ИД этих заказов, оттуда получаю количество
...
<div class="loading bx-catalog-element bx-<?=$arParams['TEMPLATE_THEME']?>" id="<?=$itemIds['ID']?>"
